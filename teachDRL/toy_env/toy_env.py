...
from teachDRL.teachers.utils.plot_utils import (
    region_plot_gif,
    gmm_plot_gif,
    random_plot_gif,
)
...
class ToyEnv(object):  # n-dimensional grid
    def __init__(self, nb_cubes=10, nb_dims=2, noise=0.0):
        ...
        self.bnds = [
            np.arange(0, 1 + self.step_size, self.step_size) for i in range(nb_dims)
        ]
        self.params = []
    ...
    def get_score(
        self,
    ):  # Returns the percentage of "mastered" hypercubes (A cube is "mastered" if its competence >75)
    ...
    def episode(self, param: np.ndarray) -> float:
        ...
        arr_p = np.array([p])
        cubes = sp.binned_statistic_dd(
            arr_p, np.ones(arr_p.shape), "count", bins=self.bnds
        ).statistic
        ...
        if all(
            ...
            previous_neighbors_idx = np.array(np.meshgrid(*prev_cube_idx)).T.reshape(
                -1, len(prev_cube_idx)
            )
            for pn_idx in previous_neighbors_idx:
                prev_idx = tuple(pn_idx)
                if all(
                    v == cube_idx[i] for i, v in enumerate(prev_idx)
                ):  # Original hypercube, not previous neighbor
                    continue
                else:
                    if self.cube_competence[prev_idx] >= (
                        3 * (self.max_per_cube / 4)
                    ):  # Previous neighbor with high comp
                        self.cube_competence[cube_idx] = min(
                            self.cube_competence[cube_idx] + 1, self.max_per_cube
                        )
                        break
        normalized_competence = np.interp(
            self.cube_competence[cube_idx], (0, self.max_per_cube), (0, 1)
        )
        # if self.noise >= 0.0:
...
def test_riac(
    env, nb_episodes, gif=True, nb_dims=2, score_step=1000, verbose=True, params={}
):
    # Init teacher
    task_generator = RIAC(
        np.array([0.0] * nb_dims), np.array([1.0] * nb_dims), params=params
    )
    ...
    if gif and nb_dims == 2:
        ...
        region_plot_gif(
            ...
            alps,
            iterations,
            task_generator.sampled_tasks,
            gifname="riac_" + str(time.time()),
            ep_len=[1] * nb_episodes,
            rewards=rewards,
            gifdir="toy_env_gifs/",
        )
...
def test_random(
    env, nb_episodes, nb_dims=2, gif=False, score_step=1000, verbose=True, params={}
):
    scores = []
    ...
    if gif and nb_dims == 2:
        ...
        print("Creating gif...")
        random_plot_gif(
            bk, gifname="random_" + str(time.time()), gifdir="toy_env_gifs/"
        )
        print("Done (see graphics/toy_env_gifs/ folder)")
    return scores
